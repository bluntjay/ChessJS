let main = {
  ...
  methods: {
    ...
    b_options: function (position,coordinates) {
      ...
      let flag = false;
      // Converts the (x, y) into grid id coordinates
      coordinates = coordinates.map(function(val){ 
          return (parseInt(position.x) + parseInt(val.x)) + '_' + (parseInt(position.y) + parseInt(val.y));
        }).filter(val => {
          let pos = { x: 0, y: 0 };
          pos.x = parseInt(val.split('_')[0]);
          pos.y = parseInt(val.split('_')[1]);
  		// If piece is not out of bounds, return the coordinate;
          if (!(pos.x < 1) && !(pos.x > 8) && !(pos.y < 1) && !(pos.y > 8)) { 
            return val;
          }
          // Determines the line-of-sight movement options for bishop/rook/queen
        }).filter(val => { 
          if (flag == false){
            if ($('#' + val).attr('chess') == 'null'){
              return val;
    ...
    endturn: function(){
      ...
      if (main.variables.turn == 'w') {
        ...
        main.methods.togglehighlight(main.variables.highlighted);
        main.variables.highlighted.length = 0;
        // Sets the selected piece to '' again
        main.variables.selectedpiece = '';
        ...
        main.methods.togglehighlight(main.variables.highlighted);
        main.variables.highlighted.length = 0;
        // Sets the selected piece back to '' again
        main.variables.selectedpiece = '';
    ...
    togglehighlight: function(options) {
      options.forEach(function(element, index, array) {
        $('#' + element).toggleClass("green shakeLittle neonGreenText");
      });
    },
    ...
    if (main.variables.selectedpiece == '' && target.name.slice(0,1) == main.variables.turn) { 
      ...
      if (selectedpiece.name == 'w_king' || selectedpiece.name == 'b_king'){
        ...
        if (t0 && t2 && t4 &&t6){ 
  		...
  		// Else if seleced piece is not white/black king than move
      } else { 
      ...
      // If it's not trying to capture pieces in its movement range
